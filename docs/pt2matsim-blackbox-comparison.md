...
(`osmConfig_toronto.xml`, same default scope/params as the Luxembourg run):
...
To regenerate the reference: run `org.matsim.pt2matsim.run.Osm2MultimodalNetwork` on `toronto.osm` with
the same config used for `lux.osm` (requires Java 25 and the pt2MATSim jar).
