...
| nodes | 1,176 | 759 |
| links | 2,465 | 1,407 |
| mean link length | 61.4 m | 105.5 m |

(Mean link length is the arithmetic mean over all links: `151,450.17 m / 2,465 = 61.44 m` for
our parity-preset network, so **61.4 m** is correct; an earlier revision rounded this to 61.6 m.)
...
The pt2MATSim column is the same reference used in the row above: its output XML was **reused from
the compare workspace** (`/home/ashraf/git/matsim-compare/p2m_toronto_network.xml`) and was **not
regenerated for this lane run**. To regenerate it externally, use the checked-in black-box harness
`tools/oracle/run_reference_oracle.sh` (pt2MATSim, GPL, invoked out of process only). The harness
asks pt2MATSim to generate its own default config at runtime and overrides only the three paths it
needs (`osmFile`, `outputNetworkFile`, `outputCoordinateSystem`); no external config is vendored:

```bash
tools/oracle/run_reference_oracle.sh /tmp/toronto.osm /tmp/p2m_toronto_network.xml
# ORACLE_PROJECT defaults to $HOME/git/pt2matsim; set ORACLE_CLASSPATH/ORACLE_HEAP to reuse a build.
```

The pt2MATSim Toronto reference was **not** regenerated during Task 9; the numbers below are the
reused workspace artifact.
...
The fixture has 96 ways carrying a `turn:lanes*` variant (64 exact `turn:lanes`, plus 20
`turn:lanes:backward` and 12 `turn:lanes:forward`), which is the source of the small `present` tail;
the overwhelming majority of links have no lane tagging at all and therefore get the spec-mandated
...
`laneDefinitions.xml`; no production code changed by this docs task). Verbatim tail of the build log:
...
- **Checkstyle: 0 violations.** 99 *warnings* are still reported (`failsOnViolation=false`, so the
  build is green). No new warnings are introduced by this branch's own lane/mapping files; the 2
  warnings in `mapping/TransitScheduleClipper.java` are pre-existing. The remainder are pre-existing
  style nits concentrated in `GtfsFeed.java` (28), `XmlSupport.java` (22), and `GtfsImporter.java` (9).
- **SpotBugs: 0 High-severity findings.** The plugin is configured at `threshold=High` with
  `failOnViolation=false`; the gate emitted no findings (the log prints no summary line, so this is
  the plugin's `check` outcome, not a copied console line).
...
# pt2MATSim reference (optional; reused from the compare workspace in Task 9, not regenerated):
tools/oracle/run_reference_oracle.sh /tmp/toronto.osm /tmp/p2m_toronto_network.xml
# The harness names pt2MATSim (GPL), generates its own default config at runtime, and overrides
# only osmFile / outputNetworkFile / outputCoordinateSystem. No external config is vendored.
```
